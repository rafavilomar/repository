...
const ProjectDetails: React.FC<props> = ({ match }) => {
  ...
  const get: ReactNode = (value: number, arr: any[]) => {
    setColums(value);
    return arr.map((image) => <img src={image} alt="example" />);
  };
        ...
        <>
          ...
          <section className="project">
            <div className="project__data">
              <h3 className="txt mainTitle-secondary">
                {projectDetails?.name}
              </h3>
              <h5 className="txt secondaryTitle">
                {projectDetails?.category} - {projectDetails?.date}
              </h5>
              <div>
                {projectDetails?.behance && (
                  <IconButton
                    variant="secondary"
                    externalURL
                    url={projectDetails?.behance}
                    icon={<LogoDribbble />}
                  />
                )}
                ...
                {projectDetails?.live && (
                  <Button value="Live" externalURL url={projectDetails?.live} />
                )}
              </div>
            </div>
            <img src={projectDetails?.presentationImg} alt="presentation" />
          </section>
